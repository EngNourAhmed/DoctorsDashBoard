    ...
    <div class="bg-[#0c0c0c] rounded-[24px] border border-white/5 p-6 md:p-8 mb-8" style="box-shadow: 0 10px 40px -10px rgba(0,0,0,0.5);">
        <div class="flex flex-col md:flex-row md:items-start justify-between gap-6">
            <div class="space-y-3">
                ...
                @endif
                <p class="text-gray-400 text-[11px] font-medium tracking-wide mt-2">Uploaded on {{ $reports->first()->created_at->format('Y-m-d H:i') }}</p>
            </div>
                    ...
                    <div class="flex items-center gap-3">
                        <button type="button"
                            ...
                            onclick="window.openBHPreview({
                                ...
                                downloadUrl: '{{ route('user.reports.download', $report) }}',
                                mime: '{{ $report->mime_type }}',
                                title: '{{ addslashes($report->original_name) }}',
