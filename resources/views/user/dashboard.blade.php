            ...
            <div class="rounded-2xl bg-slate-900/80 border border-slate-700/70 p-5 text-xs">
                ...
                <div class="space-y-3">
                    ...
                    @foreach ($latestNotifications as $notification)
                        ...
                        <div class="flex items-start gap-3 rounded-xl border border-slate-700/50 bg-slate-900/40 px-3 py-2.5">
                            <div class="h-7 w-7 flex items-center justify-center rounded-full bg-amber-400/10 border border-amber-400/20 text-amber-400">
                                <i data-lucide="bell" class="w-3.5 h-3.5"></i>
                            </div>
                            <div class="flex-1 min-w-0">
                                <div class="flex items-center justify-between gap-2">
                                    <p class="text-xs font-bold text-slate-100">{{ $data['title'] ?? 'System Update' }}</p>
                                    <span class="text-[11px] text-slate-500">{{ $notification->created_at->diffForHumans() }}</span>
                                </div>
                                <p class="mt-0.5 text-[11px] text-slate-400 line-clamp-1">{!! strip_tags($data['message'] ?? '') !!}</p>
                            </div>
                        </div>
                    @endforeach
